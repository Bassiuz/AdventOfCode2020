...
fs.readFile('day5/input.txt', (err, data) => {
    ...
    seatList.sort(function(a, b) { return a[2] - b[2]; });

    for (i = 1; i < seatList.length; i++) {
        previousSeat = seatList[i - 1];
        currentSeat = seatList[i];

        if (currentSeat[2] - previousSeat[2] == 2) {
            console.log("Found my seat, between: ", currentSeat, previousSeat);
        }
    }
